...
def create_batch_file(dir2):
    fileID = open("run_data/runme.bat","w");
    fileID.write("echo 0 > program_status.txt\n");
    fileID.write("mpiexec -np 4 lmp_mpi -in liquid_run_273.in\n");
    fileID.write("mpiexec -np 4 lmp_mpi -in liquid_run_323.in\n");
    fileID.write("mpiexec -np 4 lmp_mpi -in liquid_run_373.in\n");
    fileID.write("mpiexec -np 4 lmp_mpi -in liquid_run_423.in\n");
    ...
    fileID.write("cd %s/run_data\n" % dir2);
    fileID.write("start %s/run_data/runme.bat\n" % dir2);
    fileID.close();
    #---
...
def create_vapor_run_script(dt,Tref,equ_run,prod_run,x):
    ...
    fileID.write("pair_style morse/smooth/linear 18.000\n");
    fileID.write("read_data vapor.data\n");
    fileID.write("pair_coeff * * %.4f %.4f %.4f\n" % (x[0], x[1], x[2]));
...
def create_liquid_run_script(dt,Tref,equ_run,prod_run,x):
...
def run_all_lammps(dir2):
    cmd = "call " + dir2 + "/master.bat";
...
def cost_estimate(x,n,dt,equ_run,prod_run,typee,T,dir2):
    for Tref in T:
        create_liquid_run_script(dt,Tref,equ_run,prod_run,x);
        create_vapor_run_script(dt,Tref,equ_run,prod_run,x);
    
    run_all_lammps(dir2);
    fileID = open("results/results.txt","a");
    if typee == 1:  fileID.write("\nInitial simplex:\n");
    if typee == 2:  fileID.write("\nReflection:\n");
    if typee == 3:  fileID.write("\nExpansion:\n");
    if typee == 4:  fileID.write("\nContraction:\n");
    if typee == 5:  fileID.write("\nShrink:\n");
    fileID.close();
...
def main():
    ...
    term_tol = 1e-2; MAX_ITER = 200; cur_iter = 0;
    ...
    equ_run = 50000;
    prod_run = 50000;
    x = [1.572749, 0.992873, 5.431679];

    create_batch_file(dir2);
    ...
    for i in range(n+1): # from x_i to x_n+1
        cost_mat[i] = cost_estimate(x_mat[i][:],n,dt,equ_run,prod_run,1,T,dir2);
    ...
    while stddev > term_tol and cur_iter < MAX_ITER: # termination criteria
        ...
        cost_r = cost_estimate(x_r,n,dt,equ_run,prod_run,2,T,dir2);
        if cost_mat[0] <= cost_r and cost_r < cost_mat[n]:
            ...
            if cost_r < cost_mat[0]:
                ...
                cost_e = cost_estimate(x_e,n,dt,equ_run,prod_run,3,T,dir2);
                if cost_e < cost_r:
            ...
            #step 5 Contraction
                ...
                cost_c = cost_estimate(x_c,n,dt,equ_run,prod_run,4,T,dir2);
                ...
                #step 6 Shrink
                    ...
                    for i in range(1,n+1): # from x_2 to x_n+1
                        ...
                        cost_mat[i] = cost_estimate(x_mat[i][:],n,dt,equ_run,prod_run,5,T,dir2);
        cur_iter += 1;
